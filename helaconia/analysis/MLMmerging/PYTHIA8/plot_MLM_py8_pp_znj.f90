...
100 FORMAT(/1X,&
  ...
  SUBROUTINE plot_MLM_py8_fill(wgts)
    ...
    INTEGER::i,j,l,k,kk,IHEP,NPart,NTRACKS,IFZ,NJET,NJET30,IST,ID
    REAL(KIND(1d0))::www,ydiff,ysum,JET_KTPTMIN,JET_KTRADIUS,palg
    INTEGER,PARAMETER::MAXTRACK=2048,MAXJET=2048,MAXNUM=30
    REAL(KIND(1d0)),DIMENSION(4)::P_Z
    REAL(KIND(1d0)),DIMENSION(4,MAXTRACK)::PJET,PTRACK,PPART
    ...
    DO IHEP=1,NHEP
       ...
       IF(IST.GE.71..AND.IST.LE.79.AND.((ABS(ID).GE.1.AND.ABS(ID).LE.5).OR.&
          ...
          IF(NPART.EQ.MAXTRACK)THEN
             WRITE(*,*)"ERROR: Too many parton particles (after showering before hadronization)"
             WRITE(*,*)"in plot_MLM_py8_fill, please increase MATRACK !"
             STOP
          ENDIF
       ENDIF
    ENDDO

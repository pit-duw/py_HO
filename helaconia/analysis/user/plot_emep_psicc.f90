...
!
! PLOT FILE FOR USER
! SPECIAL EXAMPLE FOR e- e+ > psi+cc~
!
!============================================================
  USE Helac_Global
...
!      data cc/'     ',' Born'/
    ...
    DO kk=1,nwgt_analysis
       ...
       CALL bookup(l+ 4,'Mcc ',0.06d0,2.6d0,8.0d0)
       IF(emep_ISR_shower.EQ.1)THEN
          CALL bookup(l+ 5,'number of photon '//weights_info(kk),&
               1d0,-0.5d0,11.5d0)
          CALL bookup(l+ 6,'fraction of hard energy '//weights_info(kk),&
  ...
  SUBROUTINE plot_end(xnorm)
    ...
    IF(topdrawer_output)THEN
       ...
       DO kk=1,nwgt_analysis
          ...
          CALL multitop(l+ 4,n_width,n_height,'Mcc ',ytit,'LIN')
          IF(emep_ISR_shower.EQ.1)THEN
             CALL multitop(l+ 5,n_width,n_height,'number of photon ',ytit,'LOG')
             CALL multitop(l+ 6,n_width,n_height,'fraction of hard energy ',ytit,'LOG')
          ENDIF
       END DO
    ...
    IF(gnuplot_output)THEN
       ...
       DO kk=1,nwgt_analysis
          ...
          CALL MGNUPLOT(l+ 4,'Mcc ',ytit,'LIN',psfile)
          IF(emep_ISR_shower.EQ.1)THEN
             CALL MGNUPLOT(l+ 5,'number of photon ',ytit,'LOG',psfile)
             CALL MGNUPLOT(l+ 6,'fraction of hard energy ',ytit,'LOG',psfile)
          ENDIF
       ENDDO
    ...
    IF(root_output)THEN
       ...
       DO kk=1,nwgt_analysis
          ...
          CALL MROOTPLOT(l+ 4,'Mcc ',ytit,'LIN',rootfile)
          IF(emep_ISR_shower.EQ.1)THEN
             CALL MROOTPLOT(l+ 5,'number of photon ',ytit,'LOG',rootfile)
             CALL MROOTPLOT(l+ 6,'fraction of hard energy ',ytit,'LOG',rootfile)
          ENDIF
       ENDDO
...
100    FORMAT(/1X,&
  ...
  SUBROUTINE plot_fill(wgts)
    ...
    ppsi=DSQRT(ppsi)
    pini(1:4)=phad(4,1:4)+phad(5,1:4)
    Mcc=pini(4)**2-pini(1)**2-pini(2)**2-pini(3)**2
    Mcc=DSQRT(Mcc)
    !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    !
  ...
  SUBROUTINE plot_fill_QEDPS(wgts)
    ...
    REAL(KIND(1d0))::exp1,exp2,ppsi,Mcc
    ...
    DO i=1,NPTCL
       ...
       ELSEIF(NLPTN(1,i).EQ.443)THEN
          ...
          ppsi=DSQRT(ppsi)
       ELSEIF(ABS(NLPTN(1,i)).EQ.4)THEN
          pini(1:4)=pini(1:4)+PLPTN(4:7,i)
       ENDIF
    ENDDO
    Mcc=pini(4)**2-pini(3)**2-pini(2)**2-pini(1)**2
    Mcc=DSQRT(Mcc)
    ...
    DO kk=1,nwgt_analysis
       ...
       CALL mfill(l+3,ppsi,www)
       CALL mfill(l+4,Mcc,www)
       CALL mfill(l+5,DBLE(nphoton),www)
       CALL mfill(l+6,frache,www)
    ENDDO
    RETURN

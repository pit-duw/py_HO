...
!============================================================
  ...
  LOGICAL,DIMENSION(40000)::plotis3d
CONTAINS
...
!      data cc/'     ',' Born'/
    ...
    DO kk=1,nwgt_analysis
       ...
       ! bookup_3d is defined in analysis/hbook/dbook-3d.f
       ...
       CALL bookup(l+ 2,'total partonic y rap  '//weights_info(kk),&
            0.2d0,-5d0,5d0)
       CALL bookup_3d(l+ 3,'y1-y2 correlations '//weights_info(kk),&
            0.2d0,-5d0,5d0,0.2d0,-5d0,5d0)
       plotis3d(l+ 3)=.TRUE.
  ...
  SUBROUTINE plot_end(xnorm)
    ...
    IF(topdrawer_output)THEN
       ...
       DO kk=1,nwgt_analysis
          ...
          ! empty for the 3-dim plots, which is still not done.
          IF(emep_ISR_shower.EQ.1)THEN
    ...
    IF(gnuplot_output)THEN
       ...
       DO kk=1,nwgt_analysis
          ...
          CALL MGNUPLOT_3D(l+ 3,'y1 ','y2 ',ytit,'LIN',psfile)
          IF(emep_ISR_shower.EQ.1)THEN
    ...
    IF(root_output)THEN
       ...
       DO kk=1,nwgt_analysis
          ...
          CALL MROOTPLOT_3D(l+ 3,'y1 ','y2 ',ytit,'LIN',rootfile)
          IF(emep_ISR_shower.EQ.1)THEN
...
100    FORMAT(/1X,&
  ...
  SUBROUTINE plot_fill(wgts)
    ...
    yftot=rapidity(pini)
    y1=rapidity(phad(3,1:4))
    y2=rapidity(phad(4,1:4))
    !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    !

...
!      data cc/'     ',' Born'/
    ...
    DO kk=1,nwgt_analysis
       ...
       CALL bookup(l+ 2,'costh of muon  '//weights_info(kk),&
            0.02d0,-1d0,1d0)
       IF(emep_ISR_shower.EQ.1)THEN
          CALL bookup(l+ 3,'number of photon '//weights_info(kk),&
               1d0,-0.5d0,11.5d0)
          CALL bookup(l+ 4,'fraction of hard energy '//weights_info(kk),&
  ...
  SUBROUTINE plot_end(xnorm)
    ...
    IF(topdrawer_output)THEN
       ...
       DO kk=1,nwgt_analysis
          ...
          ! test spin correlations
          CALL multitop(l+ 2,n_width,n_height,'costh of muon  ',ytit,'LIN')
          IF(emep_ISR_shower.EQ.1)THEN
             CALL multitop(l+ 3,n_width,n_height,'number of photon ',ytit,'LOG')
             CALL multitop(l+ 4,n_width,n_height,'fraction of hard energy ',ytit,'LOG')
          ENDIF
       END DO
    ...
    IF(gnuplot_output)THEN
       ...
       DO kk=1,nwgt_analysis
          ...
          ! test spin correlations
          CALL MGNUPLOT(l+ 2,'costh of muon  ',ytit,'LIN',psfile)
          IF(emep_ISR_shower.EQ.1)THEN
             CALL MGNUPLOT(l+ 3,'number of photon ',ytit,'LOG',psfile)
             CALL MGNUPLOT(l+ 4,'fraction of hard energy ',ytit,'LOG',psfile)
          ENDIF
       ENDDO
    ...
    IF(root_output)THEN
       ...
       DO kk=1,nwgt_analysis
          ...
          ! test spin correlations
          CALL MROOTPLOT(l+ 2,'costh of muon  ',ytit,'LIN',rootfile)
          IF(emep_ISR_shower.EQ.1)THEN
             CALL MROOTPLOT(l+ 3,'number of photon ',ytit,'LOG',rootfile)
             CALL MROOTPLOT(l+ 4,'fraction of hard energy ',ytit,'LOG',rootfile)
          ENDIF
       ENDDO

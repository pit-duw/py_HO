// -------------------------------------------------------
// By Hua-Sheng Shao April 2015
// Convert LHE to simple ROOT tree
// Code finds ALL ".lhe" files in current directory and loops through
//--------------------------------------------------------

#include "LHE2Root_fastjet_default.h"
#include <iostream>
...
int main(){
  ...
  for ( int f = 0; f < files.size(); f++ ){
    ...
    // First loop over event to read the total weight
    while ( true ){
      // Read event
	...
	// Change ParticlePtCut in HepMC2Root_fastjet.h
	if ( partMom.perp() > ParticlePtCut ){
      ...
      // Change the jet_def in HepMC2Root_fastjet.h
	...
	// Change the JetPtCut in HepMC2Root_fastjet.h

...
C      common/plot_analy/idim,n_plot
      ...
      character*5 cc(idim)
      data cc/'pp LO','cutsH','cuH10','cuH15','cuH20','cuH25','cuH30',
     $     'cutsZ','cuZ10','cuZ15','cuZ20','cuZ25','cuZ30'/
      double precision pi
     ...
     &       //weights_info(kk)//cc(i),0.02d0,115d0,135d0)
...
C      common/plot_analy/idim,n_plot
      ...
      do i=1,idim
        ...
        call multitop(l+18,2,3,'pt min',' ','LOG')
...
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      implicit none
      integer idim,n_plot,idim2
      parameter (idim=13,n_plot=20,idim2=6)
C      common/plot_analy/idim,n_plot
      integer nexternal
     ...
     $ ,yptwpb(0:3),yptwmbx(0:3),pttthmin    
      logical siq1flag,siq2flag,siq3flag,cutflag(idim2),ggflag,
     $     uuflag,ddflag,
     . ugflag,dgflag,cutflag2,cutflag3(idim2)
...
c         stop 1
c      endif
      if (ipdg(nexternal-1).ne.5) then
          write (*,*) 'error #10 in analysis_fill: '/
     &        /'only for process "p p > t t~ > w+ w- b b~ a a"'
         stop 1
      endif
      if (ipdg(nexternal).ne.-5) then
...
C      cutflag=siq1flag.and.siq2flag.and.siq3flag
      ...
      cutflag(1)=ptamin.GT.25d0.AND.etaamax.LT.2.5d0
     &     .AND.minvaa.GE.115d0.AND.minvaa.LE.135d0
      cutflag(2)=cutflag(1).AND.drmin.GT.1d0
      cutflag(3)=cutflag(1).AND.drmin.GT.1.5d0
      cutflag(4)=cutflag(1).AND.drmin.GT.2d0
      cutflag(5)=cutflag(1).AND.drmin.GT.2.5d0
      cutflag(6)=cutflag(1).AND.drmin.GT.3d0
      cutflag3(1)=ptamin.GT.25d0.AND.etaamax.LT.2.5d0
     &     .AND.minvaa.GE.81d0.AND.minvaa.LE.101d0
      cutflag3(2)=cutflag3(1).AND.drmin.GT.1d0
      cutflag3(3)=cutflag3(1).AND.drmin.GT.1.5d0
      cutflag3(4)=cutflag3(1).AND.drmin.GT.2d0
      cutflag3(5)=cutflag3(1).AND.drmin.GT.2.5d0
      cutflag3(6)=cutflag3(1).AND.drmin.GT.3d0

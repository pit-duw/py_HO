...
c the average value of I to K and will put in J the quadratic dispersion.
      ...
      ELSE
        DO L=1,NBIN(I)
          ...
          IF(HIST(I,L).EQ.0..OR.Y.EQ.0.) THEN
             HIST(K,L)=0.
           ELSE
   ...
   5     FORMAT(' ****** OPERATION ="',A1,'" UNKNOWN ********'/)
        ...
        ENDDO
      	...
      	ELSEIF(OPER.EQ.'L') THEN  
          IF(USCORE(I).EQ.0..OR.Y.EQ.0.) THEN
             USCORE(K)=0.
           ELSE
             USCORE(K)=X*LOG10(Y*USCORE(I))
           ENDIF                         
          IF(OSCORE(I).EQ.0..OR.Y.EQ.0.) THEN
             OSCORE(K)=0.
           ELSE
...
20    CONTINUE
      ...
      DO IBIN=1,NBIN(NH)
        ...
        IF(X.NE.0.) THEN
           IF(XMX.EQ.0.) THEN
              FMX = X + HIST(NE,IBIN) ! NE is the standard deviation
              FMN = X - HIST(NE,IBIN)
           ELSE
...
c--F  Add gnuplot output
      ...
      SUBROUTINE MGNUPLOT4(N,M,BTIT,LTIT,SCALE,psfile)
      IMPLICIT REAL*8 (A-H,O-Z)
      IMPLICIT INTEGER (I-N)
      CHARACTER*(*) LTIT,BTIT,SCALE,psfile
      double precision xpos,ypos,tmp
      include "dbook.inc"
 ...
 102     FORMAT(/1X,
     ...
     &' set key off')
      ...
      ELSE
         ...
         IF(.NOT.plot_2d)plot_2d=.TRUE.
      ENDIF
      xpos=HMAX(N)+(HMAX(N)-HMIN(N))*0.05d0/2.2d0
      ypos=-1d99
      ymin=1d99
      DO 2 J=1,NBIN(N)
      if (scaleplots) then         
         tmp=scalefac*HIST(N,J)
      else
         tmp=HIST(N,J)
      endif
      if (ypos.lt.tmp)ypos=tmp
      if (SCALE.EQ.'LIN')ypos=ypos*(1d0-SIGN(0.05d0,ypos))
 2    CONTINUE
      WRITE(97,101) TITLE(N)(1:istring), TRIM(BTIT),
     & TRIM(LTIT), xpos,ypos,HMIN(N),HMAX(N)
 101   FORMAT( /1x,                               
...
c      IF(HIST(N,J).EQ.0.) GO TO 1
 ...
 200   FORMAT(' e')
      ...
      subroutine MGNUPLOT(n,string1,string2,string3,psfile)
      implicit none
      integer n,n_by4,m_by4
      character*(*) string1,string2,string3,psfile
      n_by4=4*(n-1)+1
      m_by4=n_by4+3
c write the 'n' plots with the 'n+3' error bars
      call mgnuplot4(n_by4,m_by4,string1,string2,string3,psfile)
      return
      end
c--F  Add root output
      
      SUBROUTINE MROOTPLOT4(N,M,BTIT,LTIT,rootfile)
      IMPLICIT REAL*8 (A-H,O-Z)
      IMPLICIT INTEGER (I-N)
      CHARACTER*(*) LTIT,BTIT,rootfile
      CHARACTER*5 histoid
      integer nn
      include 'dbook.inc'
c--- added these variables to scale plots at intermediate steps
...
c--- set scaleplots to be false here
      ...
      nn=N/4+1
      if (nn.lt.10) then
         write(histoid, '(A2,I1,A2)') 'id', nn, '  '
      elseif ((nn.ge.10).and.(nn.lt.100)) then
         write(histoid, '(A2,I2,A1)') 'id', nn, ' '
      elseif (nn.ge.100) then
         write(histoid, '(A2,I3)') 'id', nn
      endif
...
C     &        XHIS(N,J), ', ', HIST(N,J), ');'
 ...
 1     CONTINUE
      ...
      subroutine MROOTPLOT(n,string1,string2,string3,rootfile)
      implicit none
      integer n,n_by4,m_by4
      character*(*) string1,string2,string3,rootfile
      n_by4=4*(n-1)+1
      m_by4=n_by4+3
c write the 'n' plots with the 'n+3' error bars
      call mrootplot4(n_by4,m_by4,string1,string2,rootfile)
      return
      end

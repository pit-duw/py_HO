...
CONTAINS
  SUBROUTINE calc_pp_QQ_cb
    ...
    INTEGER::itmxn,ncalln,iounit,flag=0
    REAL(KIND(1d0)),DIMENSION(4)::pmomtemp
    LOGICAL::lexist
    SAVE lunwei,lhewgt,icase
    WRITE(*,*)'                     THE BEGINNING OF HELAC-Onia'
    WRITE(*,*)'    AddOn Process: '
    WRITE(*,*)'    Crystal Ball Function for p p > Q + Qbar '
    WRITE(*,*)'======================================================================='
    WRITE(*,*)'======================================================================='
    WRITE(*,*)' '
    ! some necesarry initialization
    ! read the physical masses
    INQUIRE(FILE=TRIM(input_dir)//"masses_default.inp",EXIST=lexist)
    IF(.NOT.lexist)THEN
       PRINT *,"Warning: the file masses_default.inp does not exist ! STOP !"
       STOP
    ENDIF
    INQUIRE(FILE=TRIM(input_dir)//"masses_default.inp",OPENED=lexist)
    IF(lexist)THEN
       INQUIRE(FILE=TRIM(input_dir)//"masses_default.inp",NUMBER=iounit)
       CLOSE(UNIT=iounit)
    ENDIF
    OPEN(UNIT=udefault,FILE=TRIM(input_dir)//"masses_default.inp")
    ! open user's masses_user.inp file
    INQUIRE(FILE=TRIM(input_dir)//"masses_user.inp",EXIST=lexist)
    IF(.NOT.lexist)THEN
       PRINT *,"Warning: the file masses_user.inp does not exist ! STOP !"
       STOP
    ENDIF
    INQUIRE(FILE=TRIM(input_dir)//"masses_user.inp",OPENED=lexist)
    IF(lexist)THEN
       INQUIRE(FILE=TRIM(input_dir)//"masses_user.inp",NUMBER=iounit)
       CLOSE(UNIT=iounit)
    ENDIF
    OPEN(UNIT=uinput,FILE=TRIM(input_dir)//"masses_user.inp")
    mc=readvalue_r("mass_c",flag)
    mD0=readvalue_r("mass_D0",flag)
    mDstar=readvalue_r("mass_Dstar",flag)
    mDpm=readvalue_r("mass_Dpm",flag)
    CLOSE(UNIT=udefault)
    CLOSE(UNIT=uinput)
    ...
    IF(lexist)THEN
       OPEN(UNIT=32333,FILE=TRIM(input_dir)//"paths/lhapdfpath")
       READ(32333,'(A)')LHAPath
       CLOSE(UNIT=32333)
       i=LEN_TRIM(LHAPath)
    ...
    OPEN(UNIT=12215,FILE="./input/state.inp")
    READ(12215,*)cb_istate
    CLOSE(UNIT=12215)
    IF(cb_istate.LT.1.OR.cb_istate.GT.4)THEN
       WRITE(*,*)"ERROR:Unknown the state = ",cb_istate
       WRITE(*,*)"INFO:Please set 1(c+cbar) or 2(D0+D0bar) or 3-4(D*+-,D+-) in state.inp"
       STOP
    ...
    ELSE
       mpsi=mDpm
    ENDIF
    OPEN(UNIT=12215,FILE="./input/includeqq.inp")
    READ(12215,*)cb_includeqq
    CLOSE(UNIT=12215)
    OPEN(UNIT=12215,FILE="./input/crystalball.inp")
    READ(12215,*)cb_n,cb_ptavg,cb_kapa,cb_lam
    cb_lambdath=0
    iflh(1)=35
    iflh(2)=35
    ...
    !CALL ReadDecayInfo
    IF(nunit1.NE.6)THEN
       OPEN(UNIT=nunit1,FILE=TRIM(output_dir)//"RESULT_pp_QQ_crystalball.out")
    ENDIF
    nunit2=32
    CLOSE(nunit2)
    OPEN(nunit2,FILE=TRIM(output_dir)//'kine_pp_QQ_crystalball.out')
    nunit3=30
    CLOSE(nunit3)
    OPEN(nunit3,FILE=TRIM(tmp_dir)//'even_pp_QQ_crystalball.out',FORM='unformatted')
    ! for LHA                                                                                                                              
    CLOSE(200)
    OPEN(200,FILE=TRIM(tmp_dir)//'sample_pp_QQ_crystalball.init')
    CALL ReadElem_integer("itmax",itmxn)
    CALL ReadElem_integer("nmc",ncalln)
    WRITE(*,*)' '
    CALL Helac_mtime()
    WRITE(*,*)' '
    CALL pp_QQ_cb_VEGAS(rslt,itmxn,ncalln)
    ...
    CLOSE(21)
    CLOSE(200)
    CALL Generate_lhe_pp_QQ_cb(nhad,Nevents,icase)
  END SUBROUTINE calc_pp_QQ_cb

  SUBROUTINE pp_QQ_cb_VEGAS(rslt,itmxn,ncalln)
    IMPLICIT NONE
    ...
    CALL VEGAS(varnum,pp_QQ_cb_fxn,vfes,sd,chi2a)
    WRITE(*,*)' '
    WRITE(*,*)"====================NCALL=20K==========================="
    WRITE(*,*)" "
    CALL Helac_mtime()
    ...
    DO ii=2,10
       CALL VEGAS(varnum,pp_QQ_cb_fxn,vfes,sd,chi2a,1)
       WRITE(*,*)"ITERATION ",ii,":"
       WRITE(*,*)vfes,"+\-",sd
       WRITE(*,*)"precision:",sd/vfes
    ENDDO
    ...
    DO
       ...
       IF(NCALL/maxprint.GT.nprint)nprint=NCALL/maxprint
         ...
         CALL Helac_mtime()
         WRITE(*,*)" "
         IF(plot_output)CALL initplot_pp_QQ_cb
         CALL VEGAS(varnum,pp_QQ_cb_fxn,vfes,sd,chi2a,1)
         IF(plot_output)CALL plotout_pp_QQ_cb
         WRITE(*,*)vfes,"+\-",sd
         WRITE(*,*)"precision:",sd/vfes
         CALL DAYTIME(iyr1,imon1,iday1,ihr1,imin1,isec1,i100th1)
         iday1=iday1-iday0
...
5200  FORMAT(1P,3E20.10,I6)
    END SUBROUTINE pp_QQ_cb_VEGAS

    FUNCTION pp_QQ_cb_fxn(x,wgt)
      REAL(KIND(1d0)),DIMENSION(varnum),INTENT(IN)::x
      REAL(KIND(1d0)),INTENT(IN)::wgt
      REAL(KIND(1d0))::pp_QQ_cb_fxn
      ...
      IF(init.EQ.0)THEN
         ...
         sq=sqs*sqs
         temp1=DSQRT(sq**2-4d0*sq*mpsi**2)/(2d0*sqs)
      ...
      IF(.NOT.labeqcoll)THEN
         ...
         ENDDO
      ENDIF
      CALL strf_pdf_pp_QQ_cb(wsf)
      wdps=wdps*wsf
      IF(wdps.LE.0d0)THEN
         pp_QQ_cb_fxn=0d0
         RETURN
      ENDIF
      CALL pp_QQ_cb_hadronmom
      icut=1
      CALL Cuts_pp_QQ_cb(icut)
      IF(icut.EQ.0)THEN
         pp_QQ_cb_fxn=0d0
         RETURN
      ENDIF
      ...
      IF(lunwei2.AND.lwmax.GT.0)THEN
         w1=pp_QQ_cb_fxn*wgt ! multiply VEGAS weight
         CALL unwei_procedure_pp_QQ_cb(w1,nwri,nwmax,nwarn)
      ENDIF
      IF(plot_output)THEN
         w1=pp_QQ_cb_fxn*wgt
         CALL outfun_pp_QQ_cb(w1)
      ENDIF
      nnn=nnn+1
      IF(recmax.LT.pp_QQ_cb_fxn)recmax=pp_QQ_cb_fxn
      IF(recmin.GT.pp_QQ_cb_fxn)recmin=pp_QQ_cb_fxn
      IF(MOD(nnn,nprint).EQ.0)THEN
         ...
         ENDIF
      ENDIF
    END FUNCTION pp_QQ_cb_fxn

    SUBROUTINE strf_pdf_pp_QQ_cb(wsf)
      USE CTEQ6PDF
      USE Structf_PDFs
      IMPLICIT NONE
      INTEGER::ipp=1
      ...
      ENDIF
      wsf=glu1_ct*glu2_ct/xp1/xp2
      IF(cb_includeqq)THEN
         wsf=wsf+(u1_ct*ub2_ct+d1_ct*db2_ct+s1_ct*sb2_ct+c1_ct*cb2_ct+&
              ub1_ct*u2_ct+db1_ct*d2_ct+sb1_ct*s2_ct+cb1_ct*c2_ct)/xp1/xp2
      ENDIF
      IF(init.EQ.0)init=1
    END SUBROUTINE strf_pdf_pp_QQ_cb

    SUBROUTINE readcuts_pp_QQ_cb
      IMPLICIT NONE
      ...
      ! minimum charm pt
      ptconia=readvalue_r("minptc",flag)
      ! maximum charm pt
      maxptconia=readvalue_r("maxptc",flag)
      ! minimum bottom pt
      ptbonia=readvalue_r("minptb",flag)
      ! maximum bottom pt
      ...
      etaconia=readvalue_r("maxrapc",flag)
      ! maximum y rapidity charm
      ycconia=readvalue_r("maxyrapc",flag)
      ! minimum y rapidity charm
      ycconialow=readvalue_r("minyrapc",flag)
      ! maximum Feynman parameter xF
      xFcconia=readvalue_r("maxxFc",flag)
      ! minimum Feynman parameter xF
      xFcconialow=readvalue_r("minxFc",flag)
      ! maxmimum rapidity bottom
      etabonia=readvalue_r("maxrapb",flag)
      ! maximum y rapidity bottom
      ycbonia=readvalue_r("maxyrapb",flag)
      ! minimum y rapidity bottom
      ycbonialow=readvalue_r("minyrapb",flag)
      ! maximum Feynman parameter xF
      xFcbonia=readvalue_r("maxxFb",flag)
      ! minimum Feynman parameter xF
      xFcbonialow=readvalue_r("minxFb",flag)
      ...
      DO i=3,nhad
         ...
         IF(i1.EQ.0)THEN
            ...
            xFcut(i)=xFcq
            xFcutlow(i)=xFcqlow
         ELSEIF(charmquark(i1).OR.opencharmmeson(i1))THEN
            ! charm and open charm meson
            ptc(i)=ptconia
         ...
         ELSEIF(bottomquark(i1).OR.openbottommeson(i1))THEN
            ! bottom and open bottom meson
            ptc(i)=ptbonia
         ...
         ELSE
            WRITE(*,*)"ERROR:Unknown the final particles ",i1
            STOP
         ENDIF
      ENDDO
      DO i=3,nhad-1
      ...
      WRITE(*,*)'---------------------------------------------------'
      IF(cb_istate.EQ.1)THEN
         WRITE(*,*)'    the cuts for p p > c+cbar '
      ELSEIF(cb_istate.EQ.2)THEN
         WRITE(*,*)'    the cuts for p p > D0+D0bar '
      ELSEIF(cb_istate.EQ.3)THEN
         WRITE(*,*)'    the cuts for p p > D*+- '
      ELSE
         WRITE(*,*)'    the cuts for p p > D+- '
      ENDIF
      ...
      ENDDO
      WRITE(*,*)'---------------------------------------------------'
    END SUBROUTINE readcuts_pp_QQ_cb

    FUNCTION charmquark(i)
      IMPLICIT NONE
      INTEGER,INTENT(IN)::i
      LOGICAL::charmquark
      IF(ABS(i).EQ.7)THEN
      ...
      ELSE
         charmquark=.FALSE.
      ENDIF
    END FUNCTION charmquark

    FUNCTION bottomquark(i)
      IMPLICIT NONE
      INTEGER,INTENT(IN)::i
      LOGICAL::bottomquark
      IF(ABS(i).EQ.12)THEN
      ...
      ELSE
         bottomquark=.FALSE.
      ENDIF
    END FUNCTION bottomquark

    FUNCTION opencharmmeson(i)
      IMPLICIT NONE
      INTEGER,INTENT(IN)::i
      LOGICAL::opencharmmeson
      SELECT CASE(ABS(i))
         CASE(411,421,10411,10421,413,423,10413,&
              10423,20413,20423,415,425,431,10431,&
              433,10433,20433,435)
            opencharmmeson=.TRUE.
         CASE DEFAULT
            opencharmmeson=.FALSE.
      END SELECT
    END FUNCTION opencharmmeson

    FUNCTION openbottommeson(i)
      IMPLICIT NONE
      INTEGER,INTENT(IN)::i
      LOGICAL::openbottommeson
      SELECT CASE(ABS(i))
         CASE(511,521,10511,10521,513,523,&
              10513,10523,20513,20523,515,&
              525,531,10531,533,10533,20533,535)
            openbottommeson=.TRUE.
         CASE DEFAULT
            openbottommeson=.FALSE.
      END SELECT
    END FUNCTION openbottommeson

    SUBROUTINE Cuts_pp_QQ_cb(icut)
      IMPLICIT NONE
      ...
      ENDIF
      RETURN
    END SUBROUTINE Cuts_pp_QQ_cb

    !SUBROUTINE HO_Decay_pp_QQ_cb(wgtbr)
    !  IMPLICIT NONE
    !  INCLUDE "stdhep_pp_QQ_cb.inc"
    ...
    !        CALL HO_One_Decay_pp_QQ_cb(i,m)
    !        wgtbr=wgtbr*br
    !     ENDIF
    !  ENDDO
    !  RETURN
    !END SUBROUTINE HO_Decay_pp_QQ_cb

    !SUBROUTINE HO_One_Decay_pp_QQ_cb(imoth,idecay)
    !  USE HOVll
    ...
    !     ENDIF
    !     RETURN
    !   END SUBROUTINE HO_One_Decay_pp_QQ_cb

    SUBROUTINE pp_QQ_cb_hadronmom
      IMPLICIT NONE
      ...
      cb_hadron_pmom(4,5)=mpsi
      RETURN
    END SUBROUTINE pp_QQ_cb_hadronmom

    !SUBROUTINE cuts_Decay_pp_QQ_cb(icut)
    ...
    SUBROUTINE unwei_procedure_pp_QQ_cb(w1,nwri,nwmax,nwarn)
      IMPLICIT NONE
      ...
      IF(lwmax.EQ.2)THEN
         ...
         IF(llwri)THEN
            nwri=nwri+1
            !IF(NDecayChains.GT.0)THEN
            !   CALL unwei_writer_Decay_pp_QQ_cb
            !   Nevents=nwri
            !   RETURN
            !ENDIF
            ...
            DO i=1,nhad
               IF(opencharmmeson(iflh(i)).OR.openbottommeson(iflh(i)))THEN
                  idup=iflh(i)
               ELSE
                  idup=pdgt(iflh(i))
               ENDIF
      ...
      ENDIF
      Nevents=nwri
    END SUBROUTINE unwei_procedure_pp_QQ_cb

    !SUBROUTINE unwei_writer_Decay_pp_QQ_cb
    !  IMPLICIT NONE
    !  INCLUDE "stdhep_pp_QQ_cb.inc"
    ...
    !          px,py,pz,p0,pmass,vtime,vspin
    !  ENDDO
    !END SUBROUTINE unwei_writer_Decay_pp_QQ_cb

    SUBROUTINE Generate_lhe_pp_QQ_cb(n1,nevent,icase)
      IMPLICIT NONE
      ...
      nunit4=31
      CLOSE(nunit4)
      OPEN(nunit4,FILE=TRIM(tmp_dir)//'sample_pp_QQ_crystalball.init')
      nunit5=32
      CLOSE(nunit5)
      OPEN(nunit5,FILE=TRIM(output_dir)//'sample_pp_QQ_crystalball.lhe')
...
5100  FORMAT(1P,3E20.10,I6)
    ...
    END SUBROUTINE Generate_lhe_pp_QQ_cb
  END MODULE pp_QQ_cb

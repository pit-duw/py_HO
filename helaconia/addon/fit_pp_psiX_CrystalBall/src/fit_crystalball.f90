PROGRAM fit_crystalball
  ...
  WRITE(*,*)'    AddOn Process: '
  WRITE(*,*)'    Fit Crystal Ball Function for p p > psi + X '
  WRITE(*,*)'======================================================================='
  WRITE(*,*)'======================================================================='
  WRITE(*,*)' '
  ...
  CLOSE(UNIT=1216)
  IF(fit_istate.LT.1.OR.fit_istate.GT.5)THEN
     WRITE(*,*)"ERROR:Unknown the state = ",fit_istate
     WRITE(*,*)"INFO:Please set 1(J/psi) or 2(psi(2S))"
     WRITE(*,*)"INFO:or 3-5(chi_c0/1/2) in state.inp"
     STOP
  ENDIF
...
SUBROUTINE FCN_FIT(NPAR,GIN,F,X,IFLAG,FUTIL)
  ...
  IF(IFLAG.EQ.1)THEN
     ...
     ELSEIF(fit_istate.EQ.3)THEN
        ...
        mpsi=mchic0
     ELSEIF(fit_istate.EQ.4)THEN
        ! chi_c1
        data_filename="data_list_4.inp"
        mpsi=mchic1
     ELSE
        ! chi_c2
        data_filename="data_list_5.inp"
        mpsi=mchic2
     ENDIF

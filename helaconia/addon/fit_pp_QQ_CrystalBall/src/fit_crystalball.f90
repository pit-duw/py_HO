PROGRAM fit_crystalball
  ...
  REAL(KIND(1d0)),DIMENSION(4)::VSTRT=(/0.5d0,0.35d0,4.5d0,2d0/),&
       STP=(/0.0d0,0.0d0,0.0d0,0d0/),ARGLIS
  ...
  INTEGER::i,IERFLG,iounit,flag=0
  LOGICAL::lexist
  WRITE(*,*)'                     THE BEGINNING OF HELAC-Onia'
  WRITE(*,*)'    AddOn Process: '
  WRITE(*,*)'    Fit Crystal Ball Function for p p > Q + Qbar '
  WRITE(*,*)'======================================================================='
  WRITE(*,*)'======================================================================='
  WRITE(*,*)' '
  ! read the physical masses
  INQUIRE(FILE=TRIM(input_dir)//"fit_masses_default.inp",EXIST=lexist)
  IF(.NOT.lexist)THEN
     PRINT *,"Warning: the file fit_masses_default.inp does not exist ! STOP !"
     STOP
  ENDIF
  INQUIRE(FILE=TRIM(input_dir)//"fit_masses_default.inp",OPENED=lexist)
  IF(lexist)THEN
     INQUIRE(FILE=TRIM(input_dir)//"fit_masses_default.inp",NUMBER=iounit)
     CLOSE(UNIT=iounit)
  ENDIF
  OPEN(UNIT=udefault,FILE=TRIM(input_dir)//"fit_masses_default.inp")
  ! open user's fit_masses_user.inp file
  INQUIRE(FILE=TRIM(input_dir)//"fit_masses_user.inp",EXIST=lexist)
  IF(.NOT.lexist)THEN
     PRINT *,"Warning: the file fit_masses_user.inp does not exist ! STOP !"
     STOP
  ENDIF
  INQUIRE(FILE=TRIM(input_dir)//"fit_masses_user.inp",OPENED=lexist)
  IF(lexist)THEN
     INQUIRE(FILE=TRIM(input_dir)//"fit_masses_user.inp",NUMBER=iounit)
     CLOSE(UNIT=iounit)
  ENDIF
  OPEN(UNIT=uinput,FILE=TRIM(input_dir)//"fit_masses_user.inp")
  ...
  CLOSE(UNIT=udefault)
  CLOSE(UNIT=uinput)
  OPEN(UNIT=1316,FILE="./input/state.inp")
  READ(1316,*)fit_istate
  CLOSE(UNIT=1316)
  IF(fit_istate.LT.1.AND.fit_istate.GT.4)THEN
     WRITE(*,*)"ERROR:Unknown the state = ",fit_istate
     WRITE(*,*)"INFO:Please set 1(c+cbar) or 2(D0+D0bar) or 3-4(D*+-,D+-) in state.inp"
     STOP
  ENDIF
  OPEN(UNIT=1316,FILE="./input/includeqq.inp")
  READ(1316,*)fit_includeqq
  CLOSE(UNIT=1316)
  OPEN(UNIT=1316,FILE="./input/fit_param_card.inp")
  DO i=1,4
     READ(1316,*)VSTRT(i),STP(i),lower(i),upper(i)
  ENDDO
  CLOSE(UNIT=1316)
  ! Initialize Minuit,define I/O unit numbers
  OPEN(UNIT=1316,FILE="./output/fit_crystalball.out")
  CALL MNINIT(5,1316,7)
  ! Define parameters, set initial values
  DO i=1,4
     ! remove the limit if lower=upper=0d0
     CALL MNPARM(NPRM(i),PNAM(i),VSTRT(i),STP(i),lower(i),upper(i),IERFLG)
     IF(IERFLG.NE.0)THEN
        WRITE(1316,'(A,I2)') ' UNABLE TO DEFINE PARAMETER NO.',I
        STOP
     ENDIF
  ENDDO
  ...
  IF(lexist)THEN
     OPEN(UNIT=33333,FILE=TRIM(input_dir)//"paths/lhapdfpath")
     READ(33333,'(A)')LHAPath
     CLOSE(UNIT=33333)
     i=LEN_TRIM(LHAPath)
...
SUBROUTINE FCN_FIT(NPAR,GIN,F,X,IFLAG,FUTIL)
  ...
  IF(IFLAG.EQ.1)THEN
     ...
     ISET=0
     DO ii=1,nfile
        OPEN(UNIT=2331,FILE=TRIM(stringfile(ii)))
        CALL READ_EXPDATA(2331,ITYPE_SET,ISET,SQRTS_SET,Y_SET,NBIN_SET,ISTART_SET,XBIN,EXVAL)
        CLOSE(UNIT=2331)
     ENDDO
  ...
  IF(IFLAG.EQ.3)THEN
     ...
     OPEN(UNIT=2335,FILE="./output/fit_results.out")
     WRITE(2335,*)"number of data = ",ISTART_SET(ISET)+NBIN_SET(ISET)
     WRITE(2335,*)"kapa = ",kapa
     WRITE(2335,*)"lam = ",lam
     WRITE(2335,*)"<pt> = ",ptavg
     WRITE(2335,*)"n = ",nn
     WRITE(2335,*)"chi^2 = ",F
...
FUNCTION fit_cb_fxn(x,wgt)
  ...
  temp2=MIN(ACosh_p((fit_sqrtS**2)/(2d0*fit_sqrtS*mt1)),fit_yup)
  temp21=MAX(-ACosh_p((fit_sqrtS**2)/(2d0*fit_sqrtS*mt1)),fit_ylow)
...
FUNCTION fit_cb_fxn2(x,wgt)
  ...
  temp2=(fit_sqrtS**2)/(2d0*fit_sqrtS*mt1)

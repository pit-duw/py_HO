...
class HelpCmd(object):
    ...
    def help_quit(self):
        logger.info("-- terminates the application",'$HO:color:BLUE')
        logger.info("syntax: quit",'$HO:color:BLACK')
    
    help_EOF = help_quit

    def help_history(self):
        logger.info("-- interact with the command history.",'$HO:color:BLUE')
        logger.info("syntax: history [FILEPATH|clean|.] ",'$HO:color:BLACK')
...
class Cmd(CheckCmd, HelpCmd, CompleteCmd, BasicCmd):
    ...
    this class should be HO independent"""
    ...
    def do_save(self, line, check=True):
        ...
        os.path.exists(pjoin(os.environ['HOME'], '.ho', 'ho_configuration.txt')):
            base = pjoin(os.environ['HOME'], '.ho', 'ho_configuration.txt')
            if hasattr(self, 'ho_dir'):
                basedir = self.ho_dir
            elif not MADEVENT:
                basedir = HODIR
            else:
            ...
            base = pjoin(self.ho_dir, 'Cards', 'ho_configuration.txt')
            basedir = self.ho_dir
        else:
            if hasattr(self, 'ho_dir'):
                base = pjoin(self.ho_dir, 'Cards', 'ho_configuration.txt')
                if len(args) == 0 and os.path.exists(base):
                    self.write_configuration(base, base, self.ho_dir)
            base = pjoin(HODIR, 'input', 'ho_configuration.txt')
            basedir = HODIR
...
class CmdShell(Cmd):
    ...
    def help_shell(self):
        """help for the shell"""
        logger.info("-- run the shell command CMD and catch output",'$HO:color:BLUE')        
        logger.info("syntax: shell CMD (or ! CMD)",'$HO:color:BLACK')
